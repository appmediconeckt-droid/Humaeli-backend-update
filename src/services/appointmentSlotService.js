...
export const buildDaySlots = (ranges, date) => {
  ...
  const currentMinutes = timeMinutes(now.time);
  ...
  for (const range of ranges) {
    ...
    const applies = specificDate
      ? specificDate === date
      : range.weekday !== null &&
        range.weekday !== undefined &&
        range.weekday !== "" &&
        Number(range.weekday) === day.getUTCDay();
    if (!applies) continue;
    ...
    for (let minute = start; minute + duration <= end; minute += duration) {
      const isPast = isPastDate || (isToday && minute <= currentMinutes);
      const slot = slots.get(minute) || {
        minutes: minute,
...
export const withAppointmentSlot = async (
  { doctorId, date, time, clinicId, earliestTime = null, excludeWalkinId = null },
  create
) => {
  ...
  if (date < now.date) {
    throw slotError("Cannot book an appointment for a past date.", 400);
  }

  if (date === now.date) {
    if (minute !== null && minute <= currentMinutes) {
      throw slotError("Cannot book an appointment for a time that has already passed. Please choose a future time slot.", 400);
    }
    if (earliestTime === null || earliestTime <= currentMinutes) {
      earliestTime = currentMinutes + 1;
    }
  }
  ...
  try {
    ...
    const blockedClinics = new Set(
      ranges
        .filter(
          (range) =>
            (range.is_unavailable === true || Number(range.is_unavailable) === 1) &&
            String(range.availability_date || range.date || "").slice(0, 10) === date
        )
        .map((range) => String(range.clinic_id || ""))
    );
    ...
    // Keep token numbering stable while excluding blocked clinics and past slots from booking.
    const slots = buildDaySlots(ranges, date)
      .map((slot) => ({ ...slot, clinicIds: slot.clinicIds.filter((id) => !blockedClinics.has(id)) }))
      .filter((slot) => slot.clinicIds.length);
    const eligible = slots.filter(
      (slot) => (!clinicId || slot.clinicIds.includes(String(clinicId)) || slot.clinicIds.includes("")) && !slot.isPast
    );
    ...
    const occupied = new Set(
      [
        ...online,
        ...walkins.filter(
          (record) => !excludeWalkinId || String(record.id || record._id) !== String(excludeWalkinId)
        ),
      ]
        .filter((record) => {
          if (String(record.priority || "").toLowerCase() === "emergency" && !record.appointment_time) return false;
          return !["cancelled", "canceled", "rejected", "reject"].includes(
            String(record.appointment_status || record.status || "").toLowerCase()
          );
        })
        .map((record) => timeMinutes(record.appointment_time || (record.date ? indiaDateTime(record.date).time : "")))
    );
    const selected = time
      ? eligible.find((slot) => slot.minutes === minute)
      : eligible.find(
          (slot) => (earliestTime === null || slot.minutes >= earliestTime) && !occupied.has(slot.minutes)
        );
    if (!selected) {
      if (time && slots.find((slot) => slot.minutes === minute)?.isPast) {
        throw slotError("Cannot book an appointment for a time that has already passed. Please choose a future time slot.", 400);
      }
      throw slotError("No matching availability slot. Please choose an available date and time.");
    }
    ...
    try {
      if (locked) await connection.query("SELECT RELEASE_LOCK(?)", [key]);
    } finally {
      connection.release();
    }
  }
